...
public class ViewNewAccount {
	...
	*/
	...
	// This alert is used should the invitation code's deadline have passed
    protected static Alert alertInvitationCodeExpired = new Alert(AlertType.INFORMATION);
	...
	*/
	...
	public static void displayNewAccount(Stage ps, String ic) {
		...
		// The code is known; make sure its deadline has not passed (defense in depth in case the
		// purge above and this check straddle the deadline)
		if (theDatabase.isInvitationExpired(theInvitationCode)) {
			theDatabase.removeInvitationAfterUse(theInvitationCode);	// It can never be used
			alertInvitationCodeExpired.showAndWait();
			return;
		}
	...
	private ViewNewAccount() {
		...
		// Configure the alert used when the UserName recognizer rejects the input
		...
		// If the invitation code has expired, this alert dialog will tell the user
		alertInvitationCodeExpired.setTitle("Expired Invitation Code");
		alertInvitationCodeExpired.setHeaderText("The invitation code has expired.");
		alertInvitationCodeExpired.setContentText(
