...
public class ControllerAddRemoveRoles {
	...
	private static void setupSelectedUser() {
		...
		// Contributor - It could be at the head of the list or later in the list
		if (theDatabase.getCurrentContributorRole()) {
			if (notTheFirst)
		...
		// Viewer - It could be at the head of the list or later in the list
		if (theDatabase.getCurrentViewerRole()) {
			if (notTheFirst)
		...
		// Curator - It could be at the head of the list or later in the list
		if (theDatabase.getCurrentCuratorRole()) {
			if (notTheFirst)
	...
	protected static void performAddRole() {
		...
		if (ViewAddRemoveRoles.theAddRole.compareTo("<Select a role>") != 0) {
			...
			if (theDatabase.updateUserRole(ViewAddRemoveRoles.theSelectedUser,
				...
				// The database refused the change, so tell the Admin rather than fail silently
				ViewAddRemoveRoles.alertRoleChangeRefused.setContentText(
						"The database did not accept the change. Try again.");
				ViewAddRemoveRoles.alertRoleChangeRefused.showAndWait();
			}
		}
	}
	...
	protected static void performRemoveRole() {
		...
		if (ViewAddRemoveRoles.theRemoveRole.compareTo("<Select a role>") != 0) {
			...
			if (theDatabase.updateUserRole(ViewAddRemoveRoles.theSelectedUser, 
				...
				// The database refused the change.  The only policy reason is that this would
				// remove the Admin role from the last remaining Admin, which must never happen.
				if (ViewAddRemoveRoles.theRemoveRole.compareTo("Admin") == 0 &&
						theDatabase.getNumberOfAdmins() <= 1)
					ViewAddRemoveRoles.alertRoleChangeRefused.setContentText(
						"The system must always have at least one Admin. Give another user the "
						+ "Admin role first, then remove it from this user.");
				else
					ViewAddRemoveRoles.alertRoleChangeRefused.setContentText(
						"The database did not accept the change. Try again.");
				ViewAddRemoveRoles.alertRoleChangeRefused.showAndWait();
