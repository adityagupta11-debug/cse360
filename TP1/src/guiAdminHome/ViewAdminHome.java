...
public class ViewAdminHome {
	...
	*/
	...
	// The deadline (date and time) after which the invitation can no longer be used
	protected static Label label_InvitationDeadline = new Label("Expires on");
	protected static DatePicker datepicker_InvitationDeadline = new DatePicker();
	protected static Label label_InvitationDeadlineTime = new Label("at (HH:mm, 24-hour)");
	protected static TextField text_InvitationDeadlineTime = new TextField();
	...
	private static final int theRole = 1;		// Admin: 1; Contributor: 2; Viewer: 3; Curator: 4
	...
	*/
	...
	public static void displayAdminHome(Stage ps, User user) {
		...
		// Reset the invitation deadline to the default (DEFAULT_INVITATION_HOURS from now) and
		// refresh the counts, which may have changed since the page was last shown
		ControllerAdminHome.resetInvitationDeadline();
	...
	private ViewAdminHome() {
        ...
        button_SendInvitation.setOnAction((ignoredEvent) -> {ControllerAdminHome.performInvitation(); });
		
		// The invitation deadline: a date picker plus a 24-hour time of day.  The defaults are
		// reset each time the page is displayed (see displayAdminHome).
		setupLabelUI(label_InvitationDeadline, "Arial", 16, 100, Pos.BASELINE_LEFT, 20, 250);
		datepicker_InvitationDeadline.setStyle("-fx-font: 16 Dialog;");
		datepicker_InvitationDeadline.setPrefWidth(170);
		datepicker_InvitationDeadline.setLayoutX(130);
		datepicker_InvitationDeadline.setLayoutY(245);
		datepicker_InvitationDeadline.setEditable(false);	// Force use of the calendar popup
		setupLabelUI(label_InvitationDeadlineTime, "Arial", 16, 180, Pos.BASELINE_LEFT, 315, 250);
		setupTextUI(text_InvitationDeadlineTime, "Arial", 16, 80, Pos.CENTER, 490, 245, true);
		text_InvitationDeadlineTime.setPromptText("HH:mm");
	
		// GUI Area 4
		setupButtonUI(button_ManageInvitations, "Dialog", 16, 250, Pos.CENTER, 20, 300);
        button_ManageInvitations.setOnAction((ignoredEvent) ->
			{ControllerAdminHome.manageInvitations(); });
	
		setupButtonUI(button_SetOnetimePassword, "Dialog", 16, 250, Pos.CENTER, 20, 345);
        button_SetOnetimePassword.setOnAction((ignoredEvent) ->
			{ControllerAdminHome.setOnetimePassword(); });

		setupButtonUI(button_DeleteUser, "Dialog", 16, 250, Pos.CENTER, 20, 390);
        button_DeleteUser.setOnAction((ignoredEvent) -> {ControllerAdminHome.deleteUser(); });

		setupButtonUI(button_ListUsers, "Dialog", 16, 250, Pos.CENTER, 20, 435);
        button_ListUsers.setOnAction((ignoredEvent) -> {ControllerAdminHome.listUsers(); });

		setupButtonUI(button_AddRemoveRoles, "Dialog", 16, 250, Pos.CENTER, 20, 480);
        button_AddRemoveRoles.setOnAction((ignoredEvent) -> {ControllerAdminHome.addRemoveRoles(); });
		
		// GUI Area 5
		setupButtonUI(button_Logout, "Dialog", 18, 250, Pos.CENTER, 20, 560);
        button_Logout.setOnAction((ignoredEvent) -> {ControllerAdminHome.performLogout(); });
    
		setupButtonUI(button_Quit, "Dialog", 18, 250, Pos.CENTER, 300, 560);
        button_Quit.setOnAction((ignoredEvent) -> {ControllerAdminHome.performQuit(); });
		...
		theRootPane.getChildren().addAll(
    		...
    		label_InvitationDeadline, datepicker_InvitationDeadline,
    		label_InvitationDeadlineTime, text_InvitationDeadlineTime, line_Separator3,
