...
public class UserNameRecognizer {
	 ...
	 * @version 1.02		2026-09-02	Require an alphabetic first character, support dash,
	 * 									underscore, period, and ampersand separators, and align
	 * 									the maximum length with the 32-character FSM requirement
	 * 
	 */
	...
	private static boolean running;						// The flag that specifies if the FSM is 
														// running
	private static int userNameSize = 0;			// A UserName may not exceed 32 characters
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				// The current character is checked against A-Z and a-z.  UserNames must begin
				// with an alphabetic character, so a digit is not accepted in this state.
			...
			case 1: 
				...
				//  2: a separator that transitions to state 2 
				...
				// -, _, ., & -> State 2
						...
						currentChar == '.' || currentChar == '&') {		// Check separator
					nextState = 2;
			...
			case 2: 
				...
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
					...
					// Count the UNChar that follows the separator
					userNameSize++;
		...
		case 1:
			...
			if (userNameSize > 32) {
				...
				userNameRecognizerErrorMessage += 
					"A UserName may only contain A-Z, a-z, 0-9, and the separators " +
					"-, _, ., and &.\n";
				return userNameRecognizerErrorMessage;
			}
		...
		case 2:
			...
			if (userNameSize > 32)
				...
				userNameRecognizerErrorMessage +=
					"A separator must be followed by A-Z, a-z, or 0-9.\n";
			return userNameRecognizerErrorMessage;
